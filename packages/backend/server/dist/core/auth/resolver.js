...
import { ActionForbidden, Config, EmailAlreadyUsed, EmailTokenNotFound, EmailVerificationRequired, InvalidEmailToken, LinkExpired, SameEmailProvided, SkipThrottle, Throttle, URLHelper, } from '../../fundamentals';
...
let AuthResolver = class AuthResolver {
    ...
    async createChangePasswordUrl(userId, callbackUrl) {
        const token = await this.token.createToken(TokenType.ChangePassword, userId);
...
__decorate([
    Public(),
    Mutation(() => Boolean),
    __param(0, Args('token')),
    __param(1, Args('newPassword')),
