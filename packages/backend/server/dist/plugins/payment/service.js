...
import { ActionForbidden, CantUpdateLifetimeSubscription, Config, CustomerPortalCreateFailed, EventEmitter, OnEvent, SameSubscriptionRecurring, SubscriptionAlreadyExists, SubscriptionExpired, SubscriptionHasBeenCanceled, SubscriptionNotExists, SubscriptionPlanNotFound, UserNotFound, } from '../../fundamentals';
...
let SubscriptionService = SubscriptionService_1 = class SubscriptionService {
    constructor(config, stripe, db, scheduleManager, event, feature) {
        this.config = config;
        this.stripe = stripe;
        this.db = db;
    ...
    async listPrices(user) {
        ...
        if (user) {
            canHaveEarlyAccessDiscount = await this.feature.isEarlyAccessUser(user.id);
            canHaveAIEarlyAccessDiscount = await this.feature.isEarlyAccessUser(user.id, EarlyAccessType.AI);
        ...
        const lifetimePriceEnabled = await this.config.runtime.fetch('plugins.payment/showLifetimePrice');
    ...
    async cancelSubscription(idempotencyKey, userId, plan) {
        ...
        if (!subscriptionInDB.stripeSubscriptionId) {
            throw new CantUpdateLifetimeSubscription('Lifetime subscription cannot be canceled.');
        }
    ...
    async resumeCanceledSubscription(idempotencyKey, userId, plan) {
        ...
        if (!subscriptionInDB.stripeSubscriptionId || !subscriptionInDB.end) {
            throw new CantUpdateLifetimeSubscription('Lifetime subscription cannot be resumed.');
        }
        if (!subscriptionInDB.canceledAt) {
            throw new SubscriptionHasBeenCanceled();
        }
    ...
    async updateSubscriptionRecurring(idempotencyKey, userId, plan, recurring) {
        ...
        if (!subscriptionInDB.stripeSubscriptionId) {
            throw new CantUpdateLifetimeSubscription('Can not update lifetime subscription.');
        }
        if (subscriptionInDB.canceledAt) {
            throw new SubscriptionHasBeenCanceled();
        }
    ...
    async saveInvoice(stripeInvoice, event) {
    ...
    async saveLifetimeSubscription(user, invoice) {
        ...
        if (savedSubscription && savedSubscription.stripeSubscriptionId) {
            ...
            await this.stripe.subscriptions.cancel(savedSubscription.stripeSubscriptionId, {
                prorate: true,
            });
        }
        else {
    ...
    async getAvailablePrice(customer, plan, recurring) {
        ...
        if (plan === SubscriptionPlan.Pro) {
            ...
            const isAIEaUser = await this.feature.isEarlyAccessUser(customer.userId, EarlyAccessType.AI);
